...
| `once`                     | Run on a single host, not on every matched host: the first matched host marked `primary: true`, else the first matched.                                                                      |
...
| `notify_failure`           | Fire the after `deploy:failed` hooks when the task fails as its own CLI invocation (`whoosh <stage> <task>`), so a pipeline run outside a deploy (e.g. an ASG refresh) notifies like a failed deploy. Default `true`, set `false` to opt out.    |
