...
| `DecodeParams(map[string]any, target) error`                                                  | Untyped params -> a typed struct (YAML tags). Unknown keys are ignored.                                                                  |
| `DecodeParamsStrict(map[string]any, target) error`                                            | DecodeParams with unknown keys rejected - use it when the struct defines the whole params surface, so typos error at load.              |
| `MergeParams(base, over map[string]any) map[string]any`                                       | Layer a task's `with:` over action defaults (over wins, nested maps merge recursively, inputs untouched).                               |
| `Or[T](p *T, fallback T) T` / `OrZero[T](v, fallback T) T`                                    | Defaults for optional params: `Or` for pointer params left nil, `OrZero` for scalars whose zero value means unset.                      |
